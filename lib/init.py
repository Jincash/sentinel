...
def has_dash_conf():
    ...
    #
    # if not, print a message stating that Botcoin Core must be installed and
    # configured, including JSONRPC access in botcoin.conf
    try:
...
def main():
    ...
    if not has_dash_conf():
        print("DashCore must be installed and configured, including JSONRPC access in botcoin.conf")
        sys.exit(1)
